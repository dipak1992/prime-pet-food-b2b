            ...
            return (
              <div key={invoice.id} className="rounded-lg border border-[#e7e4dc] bg-[#fcfbf9] p-4">
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  <span className={`inline-block rounded-full px-2 py-1 text-xs font-semibold ${getStatusColor(invoice.status)}`}>
                    {invoice.status}
                  </span>
                  <span className="text-xs text-[#6b7280]">Due {formatDate(invoice.dueDate)}</span>
                </div>
                {daysUntilDue !== null && daysUntilDue > 0 && (
                  <p className="mt-2 text-xs text-[#4b5563]">Due in {daysUntilDue} days</p>
                )}
                <button
                  onClick={() => handleDownload(invoice.id)}
                  disabled={downloadingId === invoice.id}
